...
using AotResource = TestResource.Aot.Resource;
using AotSchema = TestResource.Aot.Schema;
using AotContext = TestResource.Aot.SourceGenerationContext;

namespace TestResource.Aot.Tests;

[Trait("Category", "Integration")]
public sealed class ResourceTests
{
    private readonly AotResource _resource = new(AotContext.Default);
    ...
    {
        var attr = typeof(AotResource).GetCustomAttribute<DscResourceAttribute>();

        attr.Should().NotBeNull();
        attr!.Type.Should().Be("OpenDsc.Test/AotFile");
    }
    ...
    public void DscResourceAttribute_HasDescription()
    {
        ...
        attr!.Description.Should().Be("AOT test resource for file existence.");
    }
    ...
    public void Get_ExistingFile_ReturnsExistNull()
    {
        ...
        try
        {
            var result = _resource.Get(new AotSchema { Path = tempFile });
    ...
    public void Get_NonExistentFile_ReturnsExistFalse()
    {
        ...
        var result = _resource.Get(new AotSchema { Path = nonExistentPath });
    ...
    {
        var act = () => _resource.Get(new AotSchema { Path = "trigger-json-exception.txt" });
    ...
    {
        var act = () => _resource.Get(new AotSchema { Path = "trigger-generic-exception.txt" });
    ...
    {
        var act = () => _resource.Get(new AotSchema { Path = "trigger-io-exception.txt" });
    ...
    {
        var act = () => _resource.Get(new AotSchema { Path = "trigger-directory-not-found.txt" });
    ...
    {
        var act = () => _resource.Get(new AotSchema { Path = "trigger-unauthorized-access.txt" });
    ...
    public void Test_ExistingFile_ReturnsInDesiredState()
    {
        ...
        try
        {
            var result = _resource.Test(new AotSchema { Path = tempFile });
    ...
    public void Test_NonExistentFile_ReturnsNotInDesiredState()
    {
        ...
        var result = _resource.Test(new AotSchema { Path = nonExistentPath });
    ...
    public void Test_FileExistsButWantDeleted_ReturnsNotInDesiredState()
    {
        ...
        try
        {
            var result = _resource.Test(new AotSchema { Path = tempFile, Exist = false });
    ...
    public void Set_NonExistentFile_CreatesFile()
    {
        ...
        try
        {
            _resource.Set(new AotSchema { Path = tempFile });
    ...
    public void Set_NonExistentFile_ReportsChangedProperties()
    {
        ...
        try
        {
            var result = _resource.Set(new AotSchema { Path = tempFile });
    ...
    public void Set_ExistingFileWithExistFalse_DeletesFile()
    {
        ...
        try
        {
            var result = _resource.Set(new AotSchema { Path = tempFile, Exist = false });
    ...
    public void Delete_ExistingFile_RemovesFile()
    {
        ...
        try
        {
            _resource.Delete(new AotSchema { Path = tempFile });
    ...
    public void Delete_NonExistentFile_DoesNotThrow()
    {
        ...
        var act = () => _resource.Delete(new AotSchema { Path = nonExistentPath });
    ...
    [Fact]
    public void Export_WithExactPathFilter_ReturnsSingleFile()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"export_{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        var originalExportDir = Environment.GetEnvironmentVariable("TEST_EXPORT_DIR");
        try
        {
            var file1 = Path.Combine(tempDir, "test-filter1.txt");
            var file2 = Path.Combine(tempDir, "test-filter2.txt");
            File.WriteAllText(file1, "filter test 1");
            File.WriteAllText(file2, "filter test 2");
            Environment.SetEnvironmentVariable("TEST_EXPORT_DIR", tempDir);

            var results = _resource.Export(new AotSchema { Path = file1 }).ToList();

            results.Count.Should().Be(1);
            results[0].Path.Should().Be(file1);
        }
        finally
        {
            Environment.SetEnvironmentVariable("TEST_EXPORT_DIR", originalExportDir);
    ...
    [Fact]
    public void Export_WithWildcardFilter_ReturnsMatchingFiles()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"export_{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        var originalExportDir = Environment.GetEnvironmentVariable("TEST_EXPORT_DIR");
        try
        {
            var file1 = Path.Combine(tempDir, "test-filter1.txt");
            var file2 = Path.Combine(tempDir, "test-filter2.txt");
            var other = Path.Combine(tempDir, "test-other.txt");
            File.WriteAllText(file1, "filter test 1");
            File.WriteAllText(file2, "filter test 2");
            File.WriteAllText(other, "other");
            Environment.SetEnvironmentVariable("TEST_EXPORT_DIR", tempDir);

            var pattern = Path.Combine(tempDir, "test-filter*.txt");
            var results = _resource.Export(new AotSchema { Path = pattern }).ToList();

            results.Count.Should().Be(2);
            results.Select(r => r.Path).Should().Contain(file1);
            results.Select(r => r.Path).Should().Contain(file2);
            results.Select(r => r.Path).Should().NotContain(other);
        }
        finally
        {
            Environment.SetEnvironmentVariable("TEST_EXPORT_DIR", originalExportDir);
    ...
    [Fact]
    public void Export_WithNonMatchingFilter_ReturnsEmpty()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"export_{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        var originalExportDir = Environment.GetEnvironmentVariable("TEST_EXPORT_DIR");
        try
        {
            ...
            var pattern = Path.Combine(tempDir, "test-nonexistent*.txt");
            var results = _resource.Export(new AotSchema { Path = pattern }).ToList();
    ...
    [Fact]
    public void Export_WithSingleCharWildcard_ReturnsMatchingFiles()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"export_{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        var originalExportDir = Environment.GetEnvironmentVariable("TEST_EXPORT_DIR");
