...
public sealed class PasswordPolicyTests : WindowsTestBase
{
    ...
    [Fact]
    public void Get_CurrentPolicy_HasValidProperties()
    {
        var result = _resource.Get(new PasswordPolicySchema());

        result.MinimumPasswordLength.Should().BeLessThanOrEqualTo(14);
        result.MaximumPasswordAgeDays.Should().BeLessThanOrEqualTo(999);
        result.MinimumPasswordAgeDays.Should().BeLessThanOrEqualTo(999);
        result.PasswordHistoryLength.Should().BeLessThanOrEqualTo(24);
    }
    ...
    [RequiresAdminFact]
    public void Set_MinimumPasswordLength_UpdatesPolicy()
    {
        var original = _resource.Get(new PasswordPolicySchema());
        const uint targetLength = 8;
        ...
        try
        {
            _resource.Set(new PasswordPolicySchema
            {
                MinimumPasswordLength = targetLength,
                MaximumPasswordAgeDays = original.MaximumPasswordAgeDays,
                MinimumPasswordAgeDays = original.MinimumPasswordAgeDays,
                PasswordHistoryLength = original.PasswordHistoryLength
            });
