...
- [Prerelease Versions vs Feature Flags](#prerelease-versions-vs-feature-flags)
...
> nodes. See [Prerelease Versions vs Feature
> Flags](#prerelease-versions-vs-feature-flags) to understand when prerelease
...
## Prerelease Versions vs Feature Flags
...
- Enabling opt-in behaviours that require phased adoption (e.g., enabling HTTPS,
...
compliance reports, and then widen the scope or update the `Default` parameters
when ready. There is no prerelease or stable concept — the flag is simply `true`
or `false` at each scope level.

See the [Feature Flags guide](feature-flags.md) for the full pattern including
how to declare flags, structure scope-layered parameter files, and implement two
...
behaviour should be active (a parameter/flag concern). Prerelease versions are
left for cases where the structure itself needs canary validation before any
nodes should run the new YAML at all.

```mermaid
flowchart TD
    A[Author new resource block\ngated by enableNewFeature: false] --> B[Publish as stable version\nAll nodes receive it, feature is off]
    B --> C[Enable flag in canary\nscope parameters]
    C --> D{Compliance\'checks pass?}
    D -->|Yes| E[Promote flag to\nproduction parameters]
    D -->|No| F[Revert flag in\ncanary parameters]
    F --> C
```
